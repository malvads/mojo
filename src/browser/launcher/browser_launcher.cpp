...
bool BrowserLauncher::launch(const std::string& path,
                             int                port,
                             bool               headless,
...
#ifdef _WIN32
    user_data_path = std::filesystem::temp_directory_path().string() + "\\mojo_browser_"
    ...
    std::vector<std::string> arg_strings = {path,
                                            "--headless",
                                            "--disable-gpu",
                                            "--disable-extensions",
    ...
    Logger::info("Launched headless browser: " + path
                 + " (PID: " + std::to_string(browser_proc_info.dwProcessId) + ")");
#else
    browser_pid = fork();
    if (browser_pid < 0)
        return false;
...
#ifdef _WIN32
    if (browser_proc_info.hProcess != NULL) {
        Logger::info("Closing headless browser (PID: "
                     + std::to_string(browser_proc_info.dwProcessId) + ")...");
