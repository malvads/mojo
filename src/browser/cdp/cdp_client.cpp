...
CDPClient::~CDPClient() {
    if (!ctx_->tab_id.empty()) {
        ...
        if (curl) {
            std::string url = "http://" + host_ + ":" + std::to_string(port_) + "/json/close/" + ctx_->tab_id;
            curl_easy_setopt(curl, CURLOPT_URL, url.c_str());
            curl_easy_perform(curl);
            curl_easy_cleanup(curl);
        }
    }
    if (ctx_->lws_ctx) lws_context_destroy(ctx_->lws_ctx);
}
...
static int callback_cdp(struct lws *wsi, enum lws_callback_reasons reason, void *user [[maybe_unused]], void *in, size_t len) {
...
bool CDPClient::connect() {
    ...
    if (!ctx_->wsi) return false;

    auto start_conn = std::chrono::steady_clock::now();
    while (!ctx_->connected && std::chrono::steady_clock::now() - start_conn < std::chrono::seconds(kConnectTimeout)) {
...
void CDPClient::send_message(const nlohmann::json& msg) {
    ...
    while (ctx_->write_pending && std::chrono::steady_clock::now() - start < std::chrono::seconds(3)) {
        lws_service(ctx_->lws_ctx, 50);
    }
}
