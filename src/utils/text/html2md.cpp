...
size_t ReplaceAll(string* haystack, const string& needle, const string& replacement) {
...
bool Converter::ok() const {
    return !is_in_pre_ && !is_in_list_ && !is_in_p_ && !is_in_table_ && !is_in_tag_
...
void Converter::TidyAllLines(string* str) {
    ...
    while (read < len) {
        ...
        if (in_code_block) {
            ...
            if (option.forceLeftTrim || (trim_start < trim_end && (*str)[trim_start] != '\t')) {
                while (trim_start < trim_end && std::isspace((unsigned char)(*str)[trim_start])) {
                    ++trim_start;
                }
            }
            ...
            while (trim_end > trim_start && std::isspace((unsigned char)(*str)[trim_end - 1])) {
                --trim_end;
            }
...
string Converter::ExtractAttributeFromTagLeftOf(const string& attr) {
    ...
    return tag.substr(offset_opening_quote + 1, offset_closing_quote - 1 - offset_opening_quote);
}
...
bool Converter::ParseCharInTagContent(char ch) {
    ...
    if (chars_in_curr_line_ > option.softBreak && !is_in_table_ && !is_in_list_
        && current_tag_ != kTagImg && current_tag_ != kTagAnchor && option.splitLines) {
...
void Converter::TagBlockquote::OnHasLeftClosingTag(Converter* c) {
    --c->index_blockquote;
    if (!c->md_.empty() && c->md_.length() >= 2 && c->md_.substr(c->md_.length() - 2) == "> ") {
