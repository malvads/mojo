...
std::string Url::resolve(const std::string& base, const std::string& relative) {
    ...
    if (relative[0] == '?') {
        ...
        size_t f = base.find('#');
        std::string res;
        if (q != std::string::npos) res = base.substr(0, q);
    ...
    if (colon_pos != std::string::npos && colon_pos < 10) return "";
    ...
    if (relative[0] == '/') {
        ...
        if (!baseParsed.port.empty()) auth += ":" + baseParsed.port;
        ...
        size_t f = dir.find('#');
        if (f != std::string::npos) dir = dir.substr(0, f);
...
bool Url::is_same_domain(const std::string& url1, const std::string& url2) {
    ...
    auto clean_host = [](std::string h) {
        if (!h.empty() && h.back() == '.') h.pop_back(); // Strip trailing dot
        for (char& c : h) {
