...
static bool icontains(std::string_view haystack, std::string_view needle) {
    auto it = std::search(
        haystack.begin(), haystack.end(), needle.begin(), needle.end(), [](char c1, char c2) {
...
static bool istarts_with(std::string_view text, std::string_view prefix) {
    if (text.size() < prefix.size())
        return false;
    return std::equal(prefix.begin(), prefix.end(), text.begin(), [](char c1, char c2) {
        return std::tolower(static_cast<unsigned char>(c1))
               == std::tolower(static_cast<unsigned char>(c2));
    });
}
...
static Network::Http::ErrorType map_curl_code_to_error_type(CURLcode code) {
    switch (code) {
        ...
        case CURLE_RECV_ERROR:
            return Network::Http::ErrorType::Proxy;
        case CURLE_OPERATION_TIMEDOUT:
            return Network::Http::ErrorType::Timeout;
        default:
            return Network::Http::ErrorType::Network;
    }
}
...
size_t CurlClient::write_callback(void* contents, size_t size, size_t nmemb, void* userp) {
    ...
    if (ctx->is_image && ctx->detect_image)
        return total;
...
size_t CurlClient::header_callback(char* buffer, size_t size, size_t nitems, void* userp) {
    ...
    if (header.size() < CONTENT_TYPE_HEADER.size())
        return size * nitems;
...
void CurlClient::setup_curl_options(CURL*              curl,
    ...
    for (const auto& h : req.extra_headers)
        raw = curl_slist_append(raw, h.c_str());
    std::unique_ptr<curl_slist, CurlSlistDeleter> header_list(raw);
    if (raw)
        curl_easy_setopt(curl, CURLOPT_HTTPHEADER, raw);
}

CURLcode CurlClient::perform_curl_request(CURL*              curl,
                                          const Request&     req,
                                          RequestContext&    ctx,
...
Response CurlClient::handle_response(CURLcode           res,
                                     RequestContext&    ctx,
                                     long               response_code,
    ...
    response.success = (response.status_code >= 200
                        && response.status_code < static_cast<long>(MaxCode::ClientError));
...
Response CurlClient::perform(const Request& req) {
    if (!curl_)
        return create_error_response("Failed to initialize CURL handle");
    ...
    CURLcode res =
        perform_curl_request(curl_.get(), req, ctx, proxy_, response_code, effective_url);
