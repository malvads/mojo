...
TEST(ProxyPoolTest, SelectionFairness) {
    ...
    for(int i=0; i<3; ++i) {
        ...
        if(p.has_value()) order.insert(p->url);
    }
...
TEST(ProxyPoolTest, StressMultiThreaded) {
    std::vector<std::string> proxies;
    for(int i=0; i<100; ++i) proxies.push_back("http://proxy_" + std::to_string(i));
    std::map<std::string, int> priorities = {{"http", 0}};
...
TEST(ProxyPoolTest, ExhaustionAndRecoverySim) {
    ...
    auto p = pool.get_proxy();
    if (p) pool.report(*p, false); 
    if (p) pool.report(*p, false); // Second fail should remove it
